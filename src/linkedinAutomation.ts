...
function generateLinkedInScript(firstName: string, domain: string, message: string): string {
  ...
  const script = [
    ...
    '  } else {',
    '    console.log("Direct Connect button not found, looking for More button");',
    '    var buttons = Array.from(document.querySelectorAll("button"));',
    '    var moreBtn = buttons.find(function(b) {',
    '      var text = b.innerText && b.innerText.toLowerCase().trim();',
    '      var ariaLabel = b.getAttribute("aria-label");',
    '      return text === "more" || (ariaLabel && ariaLabel.toLowerCase().includes("more"));',
    '    });',
    '    if (moreBtn) {',
    '      moreBtn.click();',
    ...
    '        var connectOption = allButtons.find(function(b) {',
    '          var text = b.innerText && b.innerText.toLowerCase().trim();',
    '          var ariaLabel = b.getAttribute("aria-label");',
    '          return text === "connect" || (ariaLabel && ariaLabel.toLowerCase().includes("connect"));',
    '        });',
    '        if (connectOption) {',
    '          connectOption.click();',
