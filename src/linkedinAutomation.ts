...
function generateLinkedInScript(firstName: string, domain: string, message: string): string {
  ...
  const script = [
    ...
    '    connectBtn.click();',
    '    console.log("Clicked visible Connect button");',
    '    clickAddNoteAndFillMessage();',
    '  } else {',
    '    if (connectBtn) console.log("Connect button found but not visible");',
    ...
    '          var text = b.innerText && b.innerText.toLowerCase().trim();',
    '          var ariaLabel = b.getAttribute("aria-label");',
    '          return (text === "connect" || (ariaLabel && ariaLabel.toLowerCase().includes("connect"))) && isVisible(b);',
    '        });',
    '        if (connectOption) {',
    '          connectOption.click();',
